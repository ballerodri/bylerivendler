...
import { CATEGORIES, STEP_LABELS, type BookingState } from "./data"
...
export default function ReservaPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        // Re-sync selected services against live catalog so price/duration
        // changes propagate to in-progress bookings.
        if (s.services) {
          const all = CATEGORIES.flatMap((c) => c.services)
          s.services = s.services
            .map((sel) => all.find((x) => x.id === sel.id) || sel)
        }
        setStateRaw(s)
      }
  ...
  // Skip step 4 (medical) when client is existing
  const handleNext = () => {
  ...
  const renderScreen = () => {
    switch (step) {
      case 0: return <Screen1Services {...screenProps} />
      case 1: return <Screen2DateTime {...screenProps} />
      case 2: return <Screen3Details {...screenProps} />
      case 3: return <Screen4Medical {...screenProps} />
      case 4: return <Screen5Confirm {...screenProps} />
      case 5:
        return (
          <Screen6Success
            state={state}
  ...
  // Avoid hydration flash by rendering the mobile shell server-side; client
  // will swap to desktop after the matchMedia probe.
  return (
    <div className="blv">
