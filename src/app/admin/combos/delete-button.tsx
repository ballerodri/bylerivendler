...
export default function ComboDeleteButton({ comboId, name }: { comboId: string; name: string }) {
  ...
  const handleDelete = () => {
    ...
    startTransition(async () => {
      const r = await deleteCombo(comboId)
      if (r.ok) {
        router.refresh()
      } else {
  ...
  if (state === "confirming") {
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
        ...
        {error && (
          <span style={{ fontSize: 11, color: "#8c463c" }}>{error}</span>
        )}
      </div>
    )
  }
