...
import { fmtMoneyCents } from "@/lib/arca/format"
...
export async function sendInvoiceEmail(data: {
  ...
  const saludo = data.firstName ? `Gracias, ${data.firstName}.` : "¡Gracias por tu compra!"
...
<body style="margin:0;padding:0;background:#f2ede6;font-family:Georgia,serif;color:#2b2623;">
  <div style="max-width:560px;margin:0 auto;padding:40px 24px;">
    <h1 style="font-size:26px;font-weight:400;margin:0 0 16px;">${saludo}</h1>
    <p style="font-size:15px;line-height:1.6;color:#4a423d;margin:0 0 20px;">
      Adjuntamos tu <strong>Factura C ${nro}</strong> del ${data.fecha} por <strong>${fmtMoneyCents(data.totalCents)}</strong>.
    </p>
